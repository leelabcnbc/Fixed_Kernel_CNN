...
def train_one_case(model, datasets, opt_config,
                   ...
                   max_epoch=20000, first_layer_nolearning = False, batch_size = 128):
    assert len(datasets) == 6
    dataset_train, dataset_test, dataset_val = generate_datasets(
        *datasets, per_epoch_train=not legacy, shuffle_train=shuffle_train, batch_size = batch_size
    )
    ...
    optimizer = get_optimizer(model, opt_config['optimizer'], first_layer_nolearning)
...
#             'convert_data_to_gpu': True,
    ...
    if return_val_perf:
        yhat_val, y_val = training.eval_wrapper(model, dataset_val,
                                                True,
                                                None)
        yhat_val = yhat_val.reshape(yhat_val.shape[0],)
        y_val = y_val.reshape(y_val.shape[0],)
        val_perform = pearsonr(yhat_val, y_val)[0]
        if np.isnan(val_perform):
            val_perform = 0
    ...
    if dataset_test is not None:
        yhat, y = training.eval_wrapper(model, dataset_test,
                                         True,
                                        None)
        return_list.append(yhat)
        yhat = yhat.reshape(yhat.shape[0],)
        y = y.reshape(y.shape[0],)
        return_list.append(pearsonr(yhat, y)[0])
